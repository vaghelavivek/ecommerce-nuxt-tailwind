...
const openModalFunc = (index , type) => {
    isModelOpen.value = true
    currentOpenImageIndex.value = index
    currentOpenModelType.value = type

    console.log('sdsdknsdkasdn');
}

</script>

<template>
    <Modal v-show="isModelOpen" :index="currentOpenImageIndex" :details="productDetails" :type="currentOpenModelType"
        @closeModel="isModelOpen = false, isLansShow = false" />
    <div class="h-full xl:grid grid-cols-2 xl:py-16 py-8" v-show="!isModelOpen">
        <div class="xl:pr-32 px-[16px]">
            <div class="h-full">
                <Carousel :thumbnails="productDetails.thumbnails" @lensShow="(val) => isLansShow = val"
                    @openModal="(index, type) => openModalFunc(index , type)" />
            </div>
        </div>
